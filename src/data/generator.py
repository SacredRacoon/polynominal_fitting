...
class DataGenerator:
    ...
    def generate(self, num_samples: int, degree: int):
        """Генерирует зашумленные данные и матрицу признаков."""
        # 1. Генерируем X равномерно
        x = np.linspace(self.left, self.right, num_samples)
        
        # 2. Истинная функция (без шума)
        y_true = np.sin(2 * np.pi * x)
        
        # 3. Добавляем гауссовский шум (scale = sqrt(variance))
        noise_std = np.sqrt(self.noise_var)
        noise = np.random.normal(self.noise_mean, noise_std, num_samples)
        y_noisy = y_true + noise
        
        # 4. Формируем матрицу признаков X_matrix (Vandermonde-like)
        # Форма: (num_samples, degree + 1)
        # Столбцы: [1, x, x^2, ..., x^degree]
    ...
    def generate_true_curve(self, num_points: int, degree: int):
        """Генерирует гладкую кривую истинной функции для красивого графика."""
